...
Veri önbelleği, belirli PHP değişkenlerini önbellekte depolama ve daha sonra önbellekten almakla ilgilidir.
Ayrıca, [sorgu önbelleği](#query-caching) gibi daha gelişmiş önbellekleme özelliklerinin temelini oluşturur
ve [sayfa önbelleği](caching-page.md).

Aşağıdaki kod, veri önbellekleme işleminin tipik bir kullanım örneğini göstermektedir, burada `$cache`
bir [önbellek bileşeni](#önbellek-bileşenleri)'ne atıfta bulunur:
